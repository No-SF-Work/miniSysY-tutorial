...
8 个 lab 会以每周一个的速度放出，lab 1 ~ lab 4 的评测和提交实验报告持续时间为五周，后续实验的评测和提交实验报告截止时间为期末统计成绩之前。每个 lab 进行到第三周时会放出示例编译器。
...
运行时库提供一系列 I/O 函数，用于在程序中表达输入/输出需求，这些库函数不用在程序中声明即可调用，因此你的编译器需要支持在调用这些库函数时直接翻译成对应的 LLVM IR 形式的调用，但不需要检查这些库函数参数的合法性。评测时评测机会将运行时库链接并进行评测，在本地调试时，你需要下载运行时库并按如下方式链接：
